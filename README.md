...
├── application/     ユースケース(迎える/ごはん/水/散歩/社会化/しつけ/予防接種/去勢/治療/1日…)・コマンド
├── infrastructure/  in-memory / SQLite アダプタ(関係マッピング＋メメント)
├── presentation/    CLI・対話TUI(DogArt/Dashboard)・状態表示(DogView)
...
`bin/kennel-tui` は、ごはん・水・散歩・社会化・なでる・歯みがき・しつけ・予防接種・去勢/避妊・
治療・1日すごす…をメニューから選んで一匹を世話する画面。犬の福祉サイン(空腹・脱水・痛み・
不安・愛着)から表情が変わり、注意サインと行動の日誌が出る。
...
世話は CLI と同じユースケース(アプリ層)を通る。たとえば「がっつり一気食い」は深胸の犬種で
胃捻転(GDV)の急性リスクになり、社会化は社会化期かどうかで伸びが変わる——といった専門知識が
そのまま振る舞いに出る。
